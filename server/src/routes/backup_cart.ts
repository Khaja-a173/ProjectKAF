...
export default async function cartRoutes(app: FastifyInstance) {
  ...
  // --------------------------------------------------------------------------
  // POST /sessions/takeaway → create a tenant-scoped take-away session (no table)
  // - No auth required; relies on X-Tenant-Id for strict RLS scoping
  // - Returns { cart_id, mode: 'takeaway' }
  // --------------------------------------------------------------------------
  app.post('/sessions/takeaway', async (req, reply) => {
    try {
      ...
      if (ENABLE_CART_DB) {
        ...
        return reply.code(201).send({ cart_id: cartRow!.id, mode: 'takeaway' });
      }
      ...
      return reply.code(201).send({ cart_id: cartId, mode: 'takeaway' });
    } catch (err: any) {
      app.log.error({ err }, 'takeaway_session_failed');
      if (err?.code === '42P01') {
        return reply.code(503).send({ error: 'Service not available', reason: 'missing_table' });
      }
      return reply.code(500).send({ error: 'failed_to_create_takeaway_session' });
    }
  });

  // --------------------------------------------------------------------------
  // POST /cart/start  → start a cart session (dine_in if tableCode, else takeaway)
  // --------------------------------------------------------------------------
  app.post('/start', async (req, reply) => {
    ...
    try {
      const tableCode = body.data.tableCode?.trim();

      if (ENABLE_CART_DB) {
        let tableId: string | null = null;
        let mode: 'dine_in' | 'takeaway' = 'takeaway';
        if (tableCode) {
          // Resolve table by code for this tenant
          const { data: tbl, error: tErr } = await app.supabase
            .from('tables')
            .select('id, code')
            ...
            .maybeSingle();
          if (tErr) throw tErr;
          if (!tbl) return reply.code(404).send({ error: 'table_not_found' });
          tableId = tbl.id;
          mode = 'dine_in';
        }

        const { data: cartRow, error: cErr } = await app.supabase
          .from('carts')
          .insert({
            tenant_id: tenantId,
            order_type: mode,
        ...
        return reply.code(201).send({ cart_id: cartRow!.id, mode, ...(tableCode ? { table_code: tableCode } : {}) });
      }

      // Memory-backed fallback
      const id = `cart_${uuidLike()}`;
      const mode: 'dine_in' | 'takeaway' = body.data.tableCode ? 'dine_in' : 'takeaway';
      const mem: MemoryCart = {
        id,
        tenant_id: tenantId,
        table_id: null, // not tracking tables in memory fallback
        mode,
        items: [],
        created_at: new Date().toISOString(),
        updated_at: new Date().toISOString()
      ...
      memStore.set(id, mem);
      return reply.code(201).send({ cart_id: id, mode, ...(body.data.tableCode ? { table_code: body.data.tableCode } : {}) });
    } catch (err: any) {
  ...
  // --------------------------------------------------------------------------
  app.post('/orders/confirm', async (req, reply) => {
