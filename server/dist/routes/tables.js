...
const tablesRoutes = (fastify, _opts, done) => {
    ...
    fastify.get('/', async (req, rep) => {
        const tenantId = requireTenant(req, rep);
        if (!tenantId)
            return;
    ...
    fastify.delete('/:id', async (req, rep) => {
        ...
        if (!id)
            return rep.code(400).send({ error: 'missing table id' });
        const { error, count } = await supabase
            .from('tables')
            .delete({ count: 'exact' })
            .eq('tenant_id', tenantId)
    ...
    fastify.post('/hold', async (req, rep) => {
        ...
        const { data: tableRows, error: tErr } = table_id
            ...
            : await tblQuery.eq('code', table_code);
        if (tErr) {
            fastify.log.error({ err: tErr }, 'tables hold: fetch table failed');
            return rep.code(500).send({ error: 'tables hold failed' });
        }
        const table = tableRows?.[0];
        if (!table)
            return rep.code(404).send({ error: 'table_not_found' });
        // 2) Precondition: only available can be held
        if (table.status !== 'available')
            return rep.code(409).send({ error: 'table_not_available', status: table.status });
    ...
    fastify.post('/promote', async (req, rep) => {
        ...
        const { data: tableRows, error: tErr } = table_id
            ...
            : await tblQuery.eq('code', table_code);
        if (tErr) {
            fastify.log.error({ err: tErr }, 'tables promote: fetch table failed');
            return rep.code(500).send({ error: 'tables promote failed' });
        }
        const table = tableRows?.[0];
        if (!table)
            return rep.code(404).send({ error: 'table_not_found' });
        ...
        if (uErr) {
            fastify.log.error({ err: uErr }, 'tables promote: update status failed');
            return rep.code(500).send({ error: 'tables promote failed' });
        }
    ...
    fastify.post('/release', async (req, rep) => {
        ...
        const { data: tableRows, error: tErr } = table_id
            ...
            : await tblQuery.eq('code', table_code);
        if (tErr) {
            fastify.log.error({ err: tErr }, 'tables release: fetch table failed');
            return rep.code(500).send({ error: 'tables release failed' });
        }
        const table = tableRows?.[0];
        if (!table)
            return rep.code(404).send({ error: 'table_not_found' });
        ...
        if (uErr) {
            fastify.log.error({ err: uErr }, 'tables release: update status failed');
            return rep.code(500).send({ error: 'tables release failed' });
        }
