...
export interface AccessAuditLog {
  id: string
  tenantId: string
  action: 'grant' | 'revoke' | 'temp_grant' | 'role_assign' | 'role_remove' | 'suspend' | 'restore'
  targetUserId: string
...
export const DEFAULT_ROLES: Omit<Role, 'id' | 'tenantId' | 'createdAt' | 'updatedAt'>[] = [
  {
    key: 'OWNER',
    name: 'OWNER',
    displayName: 'Owner',
...
export interface AccessControlActions {
  grantCapability: (userId: string, capability: string, locationIds?: string[]) => Promise<void>
  ...
  grantTemporaryAccess: (userId: string, capability: string, expiresAt: Date, reason: string) => Promise<void>
