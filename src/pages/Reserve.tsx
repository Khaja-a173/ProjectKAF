...
export default function Reserve() {
  ...
  function buildSuggestionTimes(baseTime?: string): string[] {
    // produce up to 6 upcoming half-hour slots as strings like "6:00 PM"
    const out: string[] = [];
    const now = new Date();
    let start = new Date(now);
    if (baseTime) {
      // parse simple "H:MM AM/PM"
      const m = baseTime.match(/^(\d{1,2}):(\d{2})\s*(AM|PM)$/i);
      if (m) {
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 mb-12">
          ...
          {availableTables && (
            <div className="mt-8">
              ...
              {/* Suggestions when nothing available */}
              {(!availableTables.length || suggestionTimes.length > 0) && (
                <div className="mt-6 rounded-xl border border-amber-200 bg-amber-50 p-4">
                  <div className="mb-2 font-semibold text-amber-800">No exact matches. Try one of these times:</div>
                  <div className="flex flex-wrap gap-2">
                    {suggestionTimes.map((t) => (
                      <button
                        key={t}
                        className={`px-3 py-2 rounded-lg text-sm border transition-colors ${selectedTime === t ? 'bg-orange-500 text-white border-orange-600' : 'bg-white text-amber-900 border-amber-300 hover:bg-amber-100'}`}
                        onClick={() => { setSelectedTime(t); void loadTables(); }}
                        type="button"
                      >
                        {t}
                      </button>
