...
export default function StaffManagement() {
  ...
  const [staff, setStaff] = useState<StaffMember[]>([
    ...
    },
    {
      ...
      status: 'active',
      roles: [
        { id: 'waiter', name: 'WAITER', displayName: 'Waiter', color: '#10B981', icon: 'Users', capabilities: ['manage_orders', 'handle_payments'], isCustom: false }
      ],
      locations: [
        { id: 'loc1', name: 'Main Location', address: '123 Main St', isActive: true }
      ],
    ...
    },
    {
      ...
      status: 'active',
      roles: [
        { id: 'chef', name: 'CHEF', displayName: 'Chef', color: '#F59E0B', icon: 'ChefHat', capabilities: ['manage_kitchen', 'view_orders'], isCustom: false }
      ],
      locations: [
        { id: 'loc1', name: 'Main Location', address: '123 Main St', isActive: true }
      ],
    ...
    },
    {
      ...
      status: 'invited',
      roles: [
        { id: 'host', name: 'HOST', displayName: 'Host', color: '#8B5CF6', icon: 'UserCheck', capabilities: ['manage_reservations', 'seat_guests'], isCustom: false }
      ],
      locations: [
        { id: 'loc1', name: 'Main Location', address: '123 Main St', isActive: true }
      ],
  ...
  const defaultRoles: Role[] = [
    ...
    { id: 'host', name: 'HOST', displayName: 'Host', color: '#8B5CF6', icon: 'UserCheck', capabilities: ['manage_reservations', 'seat_guests'], isCustom: false },
    { id: 'waiter', name: 'WAITER', displayName: 'Waiter', color: '#10B981', icon: 'Users', capabilities: ['manage_orders', 'handle_payments'], isCustom: false },
    { id: 'cashier', name: 'CASHIER', displayName: 'Cashier', color: '#059669', icon: 'DollarSign', capabilities: ['handle_payments', 'issue_refunds'], isCustom: false },
    { id: 'chef', name: 'CHEF', displayName: 'Chef', color: '#F59E0B', icon: 'ChefHat', capabilities: ['manage_kitchen', 'view_orders'], isCustom: false },
    { id: 'kds', name: 'KDS', displayName: 'Kitchen Display', color: '#EF4444', icon: 'Monitor', capabilities: ['view_orders', 'update_status'], isCustom: false },
  ...
  const filteredStaff = staff.filter(member => {
    ...
    const matchesLocation = selectedLocation === 'all' || member.locations.some(loc => loc.id === selectedLocation)
    const matchesSearch = 
        ...
        <div className="bg-white rounded-xl shadow-sm p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">{staff.filter(s => s.isOnline).length}</p>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">{staff.filter(s => s.status === 'invited').length}</p>
            </div>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredStaff.map(member => (
                <tr key={member.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-900">
                      {member.isOnline ? 'Online' : `${Math.floor((Date.now() - member.lastActive.getTime()) / (1000 * 60))}m ago`}
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <div className="flex items-center space-x-2">
                      {member.status === 'invited' && (
                        <button
                          onClick={() => handleStaffAction(member.id, 'resend-invite')}
                      ...
                      {member.status === 'active' && (
                        <button
                          onClick={() => handleStaffAction(member.id, 'suspend')}
                      ...
                      {member.status === 'suspended' && (
                        <button
                          onClick={() => handleStaffAction(member.id, 'activate')}
  ...
  const renderAssignmentsView = () => (
    <div className="space-y-6">
      <div className="bg-white rounded-xl shadow-sm p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-6">Role Assignments</h3>
            ...
            <div key={member.id} className="border border-gray-200 rounded-lg p-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-4">
                  <div className="relative">
                    {member.avatar ? (
                      <img className="h-12 w-12 rounded-full object-cover" src={member.avatar} alt="" />
                    ) : (
                ...
                <div className="flex items-center space-x-2">
                  <button className="text-blue-600 hover:text-blue-900 text-sm">Add Role</button>
                  <button className="text-gray-600 hover:text-gray-900">
              ...
              <div className="mt-4">
                <div className="flex flex-wrap gap-2 mb-3">
                  {member.roles.map(role => (
                    <div key={role.id} className="flex items-center space-x-2 bg-gray-50 rounded-lg px-3 py-2">
                      <span 
                ...
                <div className="text-sm text-gray-600">
                  <span className="font-medium">Locations:</span> {member.locations.map(loc => loc.name).join(', ')}
                </div>
              </div>
            </div>
          ))}
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="bg-green-50 rounded-lg p-4">
            <h4 className="font-semibold text-green-900 mb-3">Clocked In (3)</h4>
            <div className="space-y-2">
              {staff.filter(s => s.isOnline).map(member => (
          ...
          <div className="bg-yellow-50 rounded-lg p-4">
            <h4 className="font-semibold text-yellow-900 mb-3">Scheduled (2)</h4>
            <div className="space-y-2">
  ...
  const renderPoliciesView = () => (
    <div className="space-y-6">
      <div className="bg-white rounded-xl shadow-sm p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-6">Role Policies & Capabilities</h3>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {role.capabilities.map(capability => (
                  <div key={capability} className="flex items-center space-x-2">
                    ...
                    <span className="text-sm text-gray-700 capitalize">{capability.replace('_', ' ')}</span>
                  </div>
                ))}
              </div>
            </div>
  ...
  const renderSecurityView = () => (
    <div className="space-y-6">
      <div className="bg-white rounded-xl shadow-sm p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-6">Security & Device Management</h3>
          ...
          {staff.map(member => (
            <div key={member.id} className="border border-gray-200 rounded-lg p-4">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-2">
                  <button className="text-orange-600 hover:text-orange-900 text-sm">Force Logout</button>
                  <button className="text-red-600 hover:text-red-900 text-sm">Revoke Access</button>
                </div>
              </div>
            </div>
          ))}
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white rounded-xl shadow-sm p-6">
          ...
          <div className="mt-4">
            <span className="text-sm text-green-600">↗ -0.3m from last week</span>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-600">Orders/Staff/Hour</p>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-600">Shift Adherence</p>
              ...
              <div key={member.id} className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">{member.roles[0]?.displayName}</p>
                  </div>
                </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6">
          ...
          <div className="space-y-3">
            {defaultRoles.slice(0, 6).map(role => {
              const count = staff.filter(s => s.roles.some(r => r.id === role.id)).length
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <nav className="mb-8">
          <div className="flex space-x-8">
            ...
            <Link to="/staff-management" className="text-blue-600 border-b-2 border-blue-600 pb-2 font-medium">
              Staff Management
            </Link>
      ...
      {showInviteModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-md">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Invite New Staff Member</h3>
            <div className="space-y-4">
              <div>
