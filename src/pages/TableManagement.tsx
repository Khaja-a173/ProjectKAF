...
export default function TableManagement() {
  const [activeView, setActiveView] = useState<'floor' | 'sessions' | 'analytics' | 'settings'>('floor')
  ...
  const zones: Zone[] = [
    { id: 'main-hall', name: 'Main Hall', color: '#3B82F6', tables: 12, capacity: 48 },
    { id: 'window', name: 'Window Seating', color: '#10B981', tables: 8, capacity: 24 },
    { id: 'patio', name: 'Patio', color: '#F59E0B', tables: 6, capacity: 32 },
    { id: 'private', name: 'Private Dining', color: '#8B5CF6', tables: 3, capacity: 24 },
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'held': return <Timer className="w-4 h-4" />
      case 'occupied': return <Users className="w-4 h-4" />
      case 'cleaning': return <RotateCcw className="w-4 h-4" />
  ...
  const filteredTables = tables.filter(table => {
    ...
    const matchesSearch = table.number.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         table.session?.customerName?.toLowerCase().includes(searchTerm.toLowerCase())
            ...
            <div key={zone.id} className="bg-white rounded-xl shadow-sm p-4">
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium text-blue-600">{occupied}/{zoneTables.length}</span>
                </div>
                <div className="flex justify-between text-sm">
                ...
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div 
                    ...
                    style={{ width: `${(occupied / zoneTables.length) * 100}%` }}
                  ></div>
                </div>
              </div>
            </div>
          ...
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Elapsed</th>
                ...
                <tr key={table.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      <div className={`w-3 h-3 rounded-full mr-3 ${getStatusColor(table.status)}`}></div>
                      <div>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium text-white ${getStatusColor(table.status)}`}>
                      {getStatusIcon(table.status)}
                      <span className="ml-1 capitalize">{table.status.replace('-', ' ')}</span>
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    {table.session ? (
                      <div>
                        <div className="text-sm font-medium text-gray-900">{table.session.customerName || 'Guest'}</div>
                        <div className="text-sm text-gray-500">{table.session.partySize} guests • ${table.session.totalAmount}</div>
                      </div>
                    ) : (
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    {table.session ? (
                      <span className="text-sm text-gray-900">{table.session.elapsedTime}</span>
                    ) : (
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      {table.session?.partySize} guests • {zones.find(z => z.id === table.zone)?.name}
                    </p>
                  </div>
                </div>
                <div className="text-right">
                  <div className="text-lg font-semibold text-gray-900">${table.session?.totalAmount}</div>
                  <div className="text-sm text-gray-500">{table.session?.elapsedTime}</div>
                </div>
              </div>
              <div className="mt-4 flex items-center justify-between">
                <div className="flex items-center space-x-4">
                  <span className="text-sm text-gray-600">Waiter: {table.session?.waiter}</span>
                  <span className="text-sm text-gray-600">Orders: {table.session?.orderCount}</span>
                </div>
                <div className="flex items-center space-x-2">
                  <button className="text-blue-600 hover:text-blue-900 text-sm">Transfer</button>
                  <button className="text-orange-600 hover:text-orange-900 text-sm">Split</button>
                  <button className="text-red-600 hover:text-red-900 text-sm">Close</button>
                </div>
              </div>
            </div>
          ))}
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-sm p-6">
          ...
          <div className="mt-4">
            <span className="text-sm text-green-600">↘ -1.3% from last week</span>
          </div>
        </div>
      </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input type="number" defaultValue="15" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Cleaning Timer (minutes)</label>
                <input type="number" defaultValue="10" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
              </div>
            </div>
          </div>

          <div>
            <h4 className="text-md font-medium text-gray-900 mb-4">Zone Configuration</h4>
            <div className="space-y-3">
              {zones.map(zone => (
                <div key={zone.id} className="flex items-center justify-between p-3 border border-gray-200 rounded-lg">
                  ...
                  <div className="flex items-center space-x-2">
                    <span className="text-sm text-gray-600">{zone.tables} tables</span>
