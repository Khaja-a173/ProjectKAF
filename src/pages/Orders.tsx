...
import { getOrders, createOrder, updateOrder, cancelOrder } from '../lib/api';
...
export default function Orders() {
  ...
  const loadOrders = async () => {
    try {
      setLoading(true);
      const response = await getOrders({ since: '24h' });
  ...
  const filteredOrders = orders.filter(order => {
    const matchesSearch = order.order_number.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         order.restaurant_tables?.table_number.toLowerCase().includes(searchTerm.toLowerCase());
