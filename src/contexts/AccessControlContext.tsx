...
import { User, Role, AccessPolicy, AccessAuditLog, DASHBOARD_REGISTRY, DEFAULT_ROLES } from '../types/access'
...
let globalAccessState = {
  ...
  users: [
    ...
    },
    {
      ...
      locationIds: ['location_456'],
      capabilities: DEFAULT_ROLES.find(r => r.key === 'MANAGER')!.capabilities,
      lastActive: new Date(),
      createdAt: new Date(),
      updatedAt: new Date()
    },
    {
      ...
      locationIds: ['location_456'],
      capabilities: DEFAULT_ROLES.find(r => r.key === 'STAFF_CHEF')!.capabilities,
      lastActive: new Date(),
      createdAt: new Date(),
      updatedAt: new Date()
    }
...
const accessSubscribers: Set<(state: typeof globalAccessState) => void> = new Set()
...
export const updateGlobalAccess = (updater: (prev: typeof globalAccessState) => typeof globalAccessState) => {
...
export function AccessControlProvider({ 
  ...
  const hasCapability = (capability: string, locationId?: string): boolean => {
    ...
    if (currentUser.temporaryAccess) {
      const tempAccess = currentUser.temporaryAccess.find(ta => 
  ...
  const canAccessDashboard = (dashboardKey: string): boolean => {
    ...
    return dashboard.capabilities.some(cap => 
      cap.type === 'view' && hasCapability(cap.key)
    )
  }
