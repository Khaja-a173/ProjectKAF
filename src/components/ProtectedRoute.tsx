...
export default function ProtectedRoute({ 
  ...
  const { hasCapability, canAccessDashboard, currentUser, loading } = useAccessControl()
        ...
        <div className="max-w-md w-full bg-white rounded-xl shadow-lg p-8 text-center">
          ...
          <div className="bg-gray-50 rounded-lg p-4 mb-6">
            <div className="text-sm text-gray-600">
              <p><strong>Required:</strong> {requiredCapability}</p>
              <p><strong>Your Role:</strong> {currentUser?.roles[0]?.displayName || 'None'}</p>
            </div>
          </div>
          <Link
            to="/dashboard"
    ...
    return fallback || (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="max-w-md w-full bg-white rounded-xl shadow-lg p-8 text-center">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-4">Dashboard Restricted</h2>
          <p className="text-gray-600 mb-6">
            You don't have access to this dashboard. Contact your administrator to request access.
          </p>
          <div className="bg-gray-50 rounded-lg p-4 mb-6">
            <div className="text-sm text-gray-600">
              <p><strong>Dashboard:</strong> {requiredDashboard}</p>
              <p><strong>Your Role:</strong> {currentUser?.roles[0]?.displayName || 'None'}</p>
            </div>
          </div>
          <Link
