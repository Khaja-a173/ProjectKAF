...
// QR Scanning APIs (Public - No Auth Required)
export function resolveQR(code: string, table: string) {
  return apiRequest(`/qr/resolve?code=${encodeURIComponent(code)}&table=${encodeURIComponent(table)}`, {}, { requireAuth: false });
}
...
export function advanceKdsOrder(orderId: string, toStatus: string) {
  return apiRequest(`/kds/orders/${orderId}/advance`, { 
    method: 'POST', 
    body: JSON.stringify({ to: toStatus }) 
  });
...
export function updatePaymentConfig(payload: any) {
  return apiRequest('/payments/config', { method: 'PUT', body: JSON.stringify(payload) });
}

// Payment Processing APIs
export function createPaymentIntent(payload: any) {
  return apiRequest('/payments/intent', { method: 'POST', body: JSON.stringify(payload) });
}

export function capturePayment(payload: any) {
  return apiRequest('/payments/capture', { method: 'POST', body: JSON.stringify(payload) });
}

export function refundPayment(payload: any) {
  return apiRequest('/payments/refund', { method: 'POST', body: JSON.stringify(payload) });
}

export function splitPayment(payload: any) {
  return apiRequest('/payments/split', { method: 'POST', body: JSON.stringify(payload) });
}
