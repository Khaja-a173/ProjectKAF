...
export function useBrandingManagement(_tenantId: string, _locationId?: string) {
  ...
  const uploadAsset = useCallback(async (file: File, assetType: keyof BrandingAssets): Promise<AssetUploadResult> => {
    try {
      setUploading(true)
  ...
  const importAssetFromUrl = useCallback(async (url: string, assetType: keyof BrandingAssets): Promise<AssetUploadResult> => {
    try {
      setUploading(true)
  ...
  const updateColors = useCallback((colors: Partial<BrandingDraft['colors']>) => {
    setDraft(prev => ({
      ...prev,
      colors: {
        ...prev.colors,
  ...
  const publishBranding = useCallback(async (changelog?: string) => {
    try {
      setLoading(true)
  ...
  const previewAsset = useCallback((assetType: keyof BrandingAssets, url: string) => {
