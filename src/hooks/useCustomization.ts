  ...
  pages: [
    {
      ...
      sections: [
        {
          ...
          visible: true,
          props: {
            videoUrl: 'https://player.vimeo.com/external/371433846.sd.mp4?s=236da2f3c0fd273d2c6d9a064f3ae35579b2bbdf&profile_id=139',
            headline: 'Experience Premium Indo-Arabian Dining',
            subheadline: 'Book a table, browse our authentic menu, or order takeaway from KAF Restro',
            cta: { label: 'Book Table', href: '/book-table' },
            overlayOpacity: 0.6
          },
          createdAt: new Date(),
          updatedAt: new Date()
        },
        {
          ...
          props: {
            items: [
              ...
              { label: 'Happy Customers', value: 50, suffix: 'K+', icon: 'Users' },
              { label: 'Signature Dishes', value: 100, suffix: '+', icon: 'ChefHat' },
              { label: 'Industry Awards', value: 25, suffix: '+', icon: 'Star' }
            ]
          },
          createdAt: new Date(),
          updatedAt: new Date()
      ...
      seoMeta: {
        title: 'KAF Restro - Premium Indo-Arabian Dining',
        description: 'Experience authentic Indo-Arabian cuisine at KAF Restro. Book tables, order takeaway, and enjoy premium dining.',
        keywords: ['restaurant', 'indo-arabian', 'dining', 'food', 'takeaway']
      },
      createdAt: new Date(),
      updatedAt: new Date()
    },
    {
      ...
      sections: [
        {
          ...
          visible: true,
          props: {
            videoUrl: 'https://player.vimeo.com/external/371433846.sd.mp4',
            headline: 'WELCOME TO BELLA VISTA GALLERY',
            subheadline: 'Experience the beauty, vibe, and timeless elegance',
            cta: { label: 'Explore Gallery', href: '#gallery-section' },
            overlayOpacity: 0.7
          },
          createdAt: new Date(),
          updatedAt: new Date()
        },
        {
          ...
          props: {
            items: [
              {
                asset: { url: 'https://images.pexels.com/photos/1640777/pexels-photo-1640777.jpeg?auto=compress&cs=tinysrgb&w=800' },
                caption: 'Signature Risotto',
                tags: ['food', 'signature']
              },
              {
                asset: { url: 'https://images.pexels.com/photos/842571/pexels-photo-842571.jpeg?auto=compress&cs=tinysrgb&w=800' },
                caption: 'Grilled Salmon',
                tags: ['food', 'healthy']
              },
              {
                asset: { url: 'https://images.pexels.com/photos/1581384/pexels-photo-1581384.jpeg?auto=compress&cs=tinysrgb&w=800' },
                caption: 'Main Dining Room',
                tags: ['interior', 'elegant']
              }
            ],
            layout: 'masonry',
      ...
      seoMeta: {
        title: 'Gallery - KAF Restro',
        description: 'Explore our beautiful restaurant gallery showcasing our dishes, interiors, and dining experience.',
        keywords: ['gallery', 'photos', 'restaurant', 'interior', 'food']
      },
      createdAt: new Date(),
      updatedAt: new Date()
    }
...
const customizationSubscribers: Set<(state: typeof globalCustomizationState) => void> = new Set()

const notifySubscribers = () => {
  console.log('🔄 Notifying customization subscribers')
  customizationSubscribers.forEach(callback => callback({ ...globalCustomizationState }))
}
...
export const sectionRegistry: Record<string, SectionSchema> = {
  hero_video: {
    type: 'hero_video',
    name: 'Hero Video',
    description: 'Full-screen video background with overlay text and call-to-action',
    icon: 'Play',
    category: 'media',
    schema: {
      type: 'object',
      ...
      items: [
        {
          ...
          answer: 'Yes, we highly recommend making reservations, especially for dinner service and weekends.'
        }
      ]
    },
...
export function useCustomization({ tenantId, locationId }: UseCustomizationProps) {
  ...
  const createPage = useCallback(async (data: Partial<Page>) => {
    try {
  ...
  const updatePage = useCallback(async (pageId: string, data: Partial<Page>) => {
    try {
  ...
  const addSection = useCallback(async (pageId: string, sectionType: string) => {
    try {
      ...
      updateGlobalCustomization(prev => ({
        ...
        pages: prev.pages.map(page => 
          page.id === pageId
            ? {
                ...page,
                sections: [...page.sections, newSection].sort((a, b) => a.sortIndex - b.sortIndex),
                updatedAt: new Date()
              }
            : page
        )
  ...
  const updateSection = useCallback(async (pageId: string, sectionId: string, data: Partial<PageSection>) => {
    try {
  ...
  const removeSection = useCallback(async (pageId: string, sectionId: string) => {
    try {
  ...
  const reorderSections = useCallback(async (pageId: string, order: Array<{ id: string; sortIndex: number }>) => {
    try {
  ...
  const publishPage = useCallback(async (pageId: string, changelog?: string) => {
    try {
      ...
      updateGlobalCustomization(prev => ({
        ...
        pages: prev.pages.map(p => 
          p.id === pageId
            ? { ...p, status: 'published', version: newVersion.version, publishedAt: new Date() }
            : p
        ),
  ...
  const uploadAsset = useCallback(async (file: File, metadata?: Partial<Asset>) => {
    try {
  ...
  const importAssetFromUrl = useCallback(async (url: string, metadata?: Partial<Asset>) => {
    try {
      ...
      const newAsset: Asset = {
        ...
        url,
        secureUrl: url,
        kind: url.includes('video') || url.includes('.mp4') ? 'VIDEO' : 'IMAGE',
        source: 'url',
        metadata: {
          width: 1920,
          height: 1080,
