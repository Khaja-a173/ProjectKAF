...
interface AccessManagementActions {
  ...
  revokeCapability: (userId: string, capability: string, reason?: string) => Promise<void>
  assignRole: (userId: string, roleKey: string, reason?: string) => Promise<void>
  removeRole: (userId: string, roleKey: string, reason?: string) => Promise<void>
  grantTemporaryAccess: (userId: string, capability: string, expiresAt: Date, reason: string, locationIds?: string[]) => Promise<void>
  suspendUser: (userId: string, reason: string) => Promise<void>
  restoreUser: (userId: string, reason?: string) => Promise<void>
  updateRole: (roleKey: string, capabilities: string[], reason?: string) => Promise<void>
  createCustomRole: (roleData: Partial<Role>) => Promise<Role>
  rollbackToVersion: (version: number, reason?: string) => Promise<void>
  bulkAssignRole: (userIds: string[], roleKey: string, reason?: string) => Promise<void>
    ...
    try {
      ...
      updateGlobalAccess(prev => {
        ...
        const updatedUsers = prev.users.map(u => {
          if (u.id === userId) {
            const newCapabilities = [...new Set([...u.capabilities, capability])]
            return { ...u, capabilities: newCapabilities, updatedAt: new Date() }
          }
          return u
        })
        ...
        const auditLog = createAuditLog(
          ...
          user.email,
          {
            ...
            after: { capabilities: [...new Set([...user.capabilities, capability])] }
          },
          reason
        )
            ...
            userOverrides: {
              ...
              [userId]: {
                ...
                grantedCapabilities: [...new Set([
                  ...(prev.policy.userOverrides[userId]?.grantedCapabilities || []),
                  capability
                ])],
                revokedCapabilities: prev.policy.userOverrides[userId]?.revokedCapabilities || [],
                locationIds,
                reason
              }
            }
  ...
  const revokeCapability = useCallback(async (
    ...
    try {
      ...
      updateGlobalAccess(prev => {
        ...
        const updatedUsers = prev.users.map(u => {
          if (u.id === userId) {
            const newCapabilities = u.capabilities.filter(c => c !== capability)
            return { ...u, capabilities: newCapabilities, updatedAt: new Date() }
          }
          return u
        })
        ...
        const auditLog = createAuditLog(
          ...
          user.email,
          {
            capability,
            before: { capabilities: user.capabilities },
            after: { capabilities: user.capabilities.filter(c => c !== capability) }
          },
          reason
        )
        ...
        return {
          ...
          policy: {
            ...
            userOverrides: {
              ...
              [userId]: {
                userId,
                grantedCapabilities: prev.policy.userOverrides[userId]?.grantedCapabilities?.filter(c => c !== capability) || [],
  ...
  const assignRole = useCallback(async (
    ...
    try {
      ...
      updateGlobalAccess(prev => {
        ...
        const updatedUsers = prev.users.map(u => {
          if (u.id === userId) {
            const newRoles = [...u.roles.filter(r => r.key !== roleKey), role]
            const newCapabilities = [...new Set([...u.capabilities, ...role.capabilities])]
            return { 
              ...u, 
              roles: newRoles, 
        ...
        const auditLog = createAuditLog(
          ...
          user.email,
          {
            ...
            after: { roles: [...user.roles.filter(r => r.key !== roleKey).map(r => r.key), roleKey] }
          },
          reason
        )
      ...
      updateGlobalAccess(prev => {
        ...
        const updatedUsers = prev.users.map(u => {
          if (u.id === userId) {
            ...
            const newCapabilities = [...new Set(newRoles.flatMap(r => r.capabilities))]
            return { 
              ...u, 
              roles: newRoles, 
              capabilities: newCapabilities,
        ...
        const auditLog = createAuditLog(
          ...
          user.email,
          {
            role: roleKey,
            before: { roles: user.roles.map(r => r.key) },
            after: { roles: user.roles.filter(r => r.key !== roleKey).map(r => r.key) }
          },
          reason
        )
      ...
      updateGlobalAccess(prev => {
        ...
        const auditLog = createAuditLog(
          ...
          user.email,
          {
            ...
            after: { temporaryAccess: [...(user.temporaryAccess || []), tempAccess] }
          },
          reason
        )
  ...
  const suspendUser = useCallback(async (userId: string, reason: string) => {
    try {
      ...
      updateGlobalAccess(prev => {
        ...
        const updatedUsers = prev.users.map(u => {
          if (u.id === userId) {
            return { ...u, status: 'suspended' as const, updatedAt: new Date() }
          }
          return u
        })
      ...
      updateGlobalAccess(prev => {
        ...
        const updatedUsers = prev.users.map(user => {
          if (user.roles.some(r => r.key === roleKey)) {
            ...
            const newCapabilities = [...new Set(userRoles.flatMap(r => r.capabilities))]
            return { 
              ...user, 
              roles: userRoles, 
              capabilities: newCapabilities,
